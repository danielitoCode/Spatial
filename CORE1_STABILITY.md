...
| 2026-07-08   | Antigravity | Fixed two critical bugs preventing rendering: (1) Added `glClear` to `applyClearColor()` in `SpatialGlRenderer.kt` so the framebuffer is actually cleared. (2) Resolved black-screen on recomposition by refactoring `rememberSceneGraph` and `SceneElement` to use Compose `DisposableEffect` and `SnapshotStateList` instead of resetting the builder top-down, preventing node list from being wiped on camera/gesture recompositions. Eagerly snapshot `pendingNodes` and `pendingCameraSnapshot` in `requestFrame()` to prevent races during GL initialization. |
...
| 2026-07-19   | Claude    | Diagnosed and fixed a `Scene(backgroundColor = Color.Transparent)` regression reported by the project owner (all colors rendered correctly except `Transparent`, which showed solid black). Root cause: at some point after the 2026-07-09 entry above (not itself logged with an audit note, so exact date/author unclear from this file), `SpatialGlSurfaceView` switched from `setZOrderOnTop(true)` to `setZOrderMediaOverlay(true)` to fix a *different* bug (3D figures rendering on top of other Compose UI, e.g. TopBars/dialogs, and figures disappearing after screen lock/unlock). That switch was the correct fix for occlusion, but had an unintended side effect: `setZOrderOnTop(true)` placed the GL surface's compositor layer above literally everything, including the hosting `Window`'s own opaque backing, so transparent clear colors showed through to whatever was behind the app. `setZOrderMediaOverlay(true)` places that layer *below* the window's main surface instead - correct for occlusion, but now a transparent clear color shows the window's own opaque (black-by-default) backing, since nothing in this codebase ever marked the hosting `Activity`'s `Window` itself as translucent (`window.setFormat(PixelFormat.TRANSLUCENT)`); only the `SurfaceHolder`'s own format was ever set. Fixed by adding `Context.findActivity()` (unwraps `ContextWrapper` layers) and `SpatialGlSurfaceView.ensureWindowTranslucentIfNeeded()`, called from `render(frame)` with the frame's actual clear color: only when `alpha < 1f` does it mark the hosting Window translucent, once per view instance, so opaque-background scenes (the common case) never pay the cost of a translucent window (which disables some SurfaceFlinger compositing optimizations). Not verified on-device in this sandbox (no Android SDK/emulator network access here) - reported and needs re-confirmation from the project owner after building. |
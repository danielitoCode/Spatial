package com.elitec.spatial_renderer.gl

import android.opengl.GLSurfaceView
import android.view.ViewGroup
import androidx.test.core.app.ActivityScenario
import androidx.test.ext.junit.runners.AndroidJUnit4
import com.elitec.spatial_core.camera.CameraSnapshot
import com.elitec.spatial_core.render.Color4
import com.elitec.spatial_core.scene.MaterialData
import com.elitec.spatial_core.scene.RenderableNode
import com.elitec.spatial_geometry.GlobalMeshRegistry
import com.elitec.spatial_geometry.MeshData
import org.junit.After
import org.junit.Assert.assertTrue
import org.junit.Test
import org.junit.runner.RunWith
import java.util.concurrent.CountDownLatch
import java.util.concurrent.TimeUnit
import java.util.concurrent.atomic.AtomicBoolean
import java.util.concurrent.atomic.AtomicReference
import javax.microedition.khronos.egl.EGLConfig
import javax.microedition.khronos.opengles.GL10

/**
 * Core #2 item 1.7 (prep) — registered dynamic mesh is visible via GlobalMeshRegistry JIT path.
 *
 * This does **not** parse a `.glb` file; it validates the renderer registry fallback that
 * `rememberModel` / `Element.Model` rely on. Full GLB asset test remains owner/device work.
 *
 * Run on physical device or emulator:
 * `./gradlew :spatial-renderer:connectedDebugAndroidTest`
 */
@RunWith(AndroidJUnit4::class)
class RegisteredModelRendersOnDeviceTest {

    private val meshId = "test:registered_triangle"

    @After
    fun tearDown() {
        GlobalMeshRegistry.unregister(meshId)
    }

    @Test
    fun registered_mesh_is_visible_on_first_frame() {
        GlobalMeshRegistry.register(meshId, MeshData.FallbackTriangle)

        val latch = CountDownLatch(1)
        val framebufferRef = AtomicReference<IntArray>()
        val captured = AtomicBoolean(false)
        val distinctiveClearColor = Color4(1f, 0f, 1f, 1f)
        val node = RenderableNode(
            meshId = meshId,
            material = MaterialData(r = 1f, g = 1f, b = 1f, a = 1f),
        )

        val scenario = ActivityScenario.launch(FirstFrameTestActivity::class.java)
        try {
            scenario.onActivity { activity ->
                val glSurfaceView = GLSurfaceView(activity)
                glSurfaceView.setEGLContextClientVersion(3)
                glSurfaceView.layoutParams = ViewGroup.LayoutParams(256, 256)

                val spatialRenderer = SpatialGlRenderer()
                spatialRenderer.updateNodes(listOf(node))
                spatialRenderer.updateCamera(CameraSnapshot(yaw = 25f, pitch = 15f, zoom = 1f))
                spatialRenderer.updateClearColor(distinctiveClearColor)

                val capturingRenderer = object : GLSurfaceView.Renderer {
                    private var width = 0
                    private var height = 0

                    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
                        spatialRenderer.onSurfaceCreated(gl, config)
                    }

                    override fun onSurfaceChanged(gl: GL10?, w: Int, h: Int) {
                        width = w
                        height = h
                        spatialRenderer.onSurfaceChanged(gl, w, h)
                    }

                    override fun onDrawFrame(gl: GL10?) {
                        spatialRenderer.onDrawFrame(gl)
                        if (captured.compareAndSet(false, true) && width > 0 && height > 0) {
                            framebufferRef.set(readFramebufferPixels(width, height))
                            latch.countDown()
                        }
                    }
                }

                glSurfaceView.setRenderer(capturingRenderer)
                glSurfaceView.renderMode = GLSurfaceView.RENDERMODE_CONTINUOUSLY
                activity.setContentView(glSurfaceView)
            }

            assertTrue(
                "Timed out waiting for first GL frame with registered mesh",
                latch.await(10, TimeUnit.SECONDS),
            )
            val pixels = requireNotNull(framebufferRef.get())
            val backgroundArgb = distinctiveClearColor.toArgbInt()
            val nonBackground = pixels.count { it != backgroundArgb }
            assertTrue(
                "Expected non-magenta pixels from GlobalMeshRegistry triangle; got clear-only frame",
                nonBackground > 0,
            )
        } finally {
            scenario.close()
        }
    }

    private fun readFramebufferPixels(width: Int, height: Int): IntArray {
        val buffer = java.nio.ByteBuffer.allocateDirect(width * height * 4)
            .order(java.nio.ByteOrder.nativeOrder())
        android.opengl.GLES30.glReadPixels(
            0, 0, width, height,
            android.opengl.GLES30.GL_RGBA,
            android.opengl.GLES30.GL_UNSIGNED_BYTE,
            buffer,
        )
        buffer.rewind()
        val out = IntArray(width * height)
        for (i in 0 until width * height) {
            val r = buffer.get().toInt() and 0xFF
            val g = buffer.get().toInt() and 0xFF
            val b = buffer.get().toInt() and 0xFF
            val a = buffer.get().toInt() and 0xFF
            out[i] = (a shl 24) or (r shl 16) or (g shl 8) or b
        }
        return out
    }

    private fun Color4.toArgbInt(): Int {
        val r = (this.r * 255f).toInt().coerceIn(0, 255)
        val g = (this.g * 255f).toInt().coerceIn(0, 255)
        val b = (this.b * 255f).toInt().coerceIn(0, 255)
        val a = (this.a * 255f).toInt().coerceIn(0, 255)
        return (a shl 24) or (r shl 16) or (g shl 8) or b
    }
}

...
<header class="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[95%] max-w-6xl">
  <nav class="glass-panel-elevated p-2 rounded-[2rem] border border-white/10 shadow-2xl shadow-black/80">
    <Row
    ...
    >
      ...
      <Item>
        ...
        >
          ...
          <div class="flex flex-col leading-tight">
            ...
            <span class="text-[8px] font-mono text-primary font-black uppercase tracking-[0.2em]">DEV_SYSTEM</span>
          </div>
        </a>
      </Item>
      ...
      <Item>
        <div class="hidden lg:block">
          <Row gap="6px" alignItems="center" class="p-1 bg-black/40 rounded-2xl border border-white/5 backdrop-blur-3xl shadow-inner">
            {#each navLinks as link}
              <Item>
                <a
                  href="#{link.id}"
                  onclick={(e) => navigate(e, link.id)}
                  class="group flex items-center gap-2 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] transition-all duration-500 {activeRoute === link.id
        ...
        <Row gap="12px" alignItems="center">
          ...
          <Item>
            <SpatialButton
              variant="primary"
              size="sm"
              class="!rounded-2xl !px-6 h-11 shadow-2xl !bg-none border-primary/30 !text-primary hover:!bg-primary/10 transition-all duration-300"
              onclick={(e: MouseEvent) => navigate(e, 'playground')}
            >
              <span class="material-symbols-outlined text-[18px]">rocket_launch</span>
              <span class="hidden sm:inline tracking-tighter uppercase">Launch Studio</span>
            </SpatialButton>
          </Item>
        </Row>
      </Item>
